...
**Input**: User description: "Prove, at demo fidelity, whether a conversational agent running inside the application can sit behind a durable work queue for a customer-messaging product: the channel is acknowledged immediately, rapid texts become one agent turn, retries and process restarts do not lose or duplicate work, a choice can pause and resume from a click, conversation identity is separate from the channel identity, human takeover silences the agent, an operator reply is visible on the next agent turn, and the operator inbox shows what was sent on the channel while agent memory keeps only a short gist. Produce a written go / no-go. Do not clone the production CRM, its database, or a live messaging integration."

## Clarifications

### Session 2026-09-14

- Q: If a customer clicks a button while a turn for that same conversation is already running, should that click wait until the current turn finishes? → A: Schedule the click right away, but run it only after the current turn for that conversation finishes.
- Q: If a conversation leaves automatic-bot after a turn was already scheduled, for example because an operator takes over, should that scheduled turn still call the agent? → A: The already-scheduled turn always runs the agent, even if the status changed.
- Q: When a customer message is accepted, should the acknowledgement include the conversation identity so takeover, close, and operator reply can target it? → A: Yes. The acknowledgement includes the conversation identity.
- Q: If the customer clicks and no choice is pending for that conversation, what should happen to that click? → A: Store the click in the inbox and do not call the agent or invent a customer text.
- Q: Must the proof include an action that sets the conversation to human-active, or is it enough that the silence rule applies if that status is already set? → A: No extra action. If the status is already human-active, new messages do not run the agent. Takeover sets waiting-for-human only.
...
**Why this priority**: Choice-and-resume is an existing product behavior that must keep working once turns move to background work. A double send, a lost pause, or a resume that looks like typed text is a no-go.
...
- **FR-041**: Takeover, release, close, operator reply, and inbound acceptance MUST be available as direct actions in the proof. Takeover, release, close, and operator reply MUST be addressable by the conversation identity returned when the customer message was accepted. A full operator console is not required.
...
- **Decision record**: The written outcome of the proof. One result and its evidence for each architecture question, an overall recommendation for agent-plus-durable-queue, and a separate recommendation for porting the existing specialist agents.
...
- Already-proven agent behaviors are reused, not rebuilt. An agent playground, extra routing agents, packaged add-on bundles, and per-organization credentials are out of scope.
